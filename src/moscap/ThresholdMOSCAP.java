...
public class ThresholdMOSCAP extends MOSCAP {
	...
	public static double Vtn(double Na,double QfQit,double Xox){
		...
		return Constants.s( Vfb(Na,Xox,QfQit)+2*phiFb(Na)+Math.sqrt(2*Constants.q*Constants.esi*Na*phiFb(Na))/Cox(Xox));
	}
	...
	public static double Qg(double Na,double QfQit){
		...
		return Constants.s( Math.sqrt(4*Constants.q*Constants.esi*Na*phiFb(Na))-QfQit);
	}
