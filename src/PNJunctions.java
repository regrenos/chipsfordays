...
public class PNJunctions {
	...
	public static double Vbi(double Na, double Nd){
		...
		return Constants.s( Constants.kbtq*Math.log((Na*Nd)/Constants.p(Constants.ni,2)));
	}
	...
	public static double Wd(double Na, double Nd,double Vpn){
		...
		return Constants.s( Math.sqrt((2*Constants.esi*(Na+Nd)*(Vbi(Na,Nd)-Vpn))/(Constants.q*Na*Nd)));
	}
	...
	public static double zeroField(double Na,double Nd,double Vpn){
		...
		return Constants.s((Constants.q*Na*Wdp(Na,Nd,Vpn))/Constants.esi);
	...
	public static double depletionCapacitance(double Na, double Nd, double Vpn){
		...
		return Constants.s( Math.sqrt((Constants.q*Constants.esi*Na*Nd)/(2*(Na+Nd)*(Vbi(Na,Nd)-Vpn))));
	}
