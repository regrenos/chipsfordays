...
public class SmallSignalMOSFET {
	...
	public static double id(double gm, double vgs, double gmb, double vbs,
			double go, double vds) {
		double result = (gm * vgs) + (gmb * vbs) + (go * vds);
		System.out
				.print("The small signal drain current for the MOSFET is (A): ");
		return Constants.s(result);
	}

	/** Small Signal Transconductance */
	public static double gmsat(double Kn, double VGS, double lambda,
			double VDS, double VBS, double Na, double Xox, double QfQit) {
	...
	public static double gmbsat(double Kn, double VGS, double lambda,
			double VDS, double VBS, double Na, double Xox, double QfQit) {
		...
		System.out
				.print("The transconductance in the bulk gmbsat (A/V^2) is: ");
		return Constants.s(result);
	}
	...
	public static double gosat() {
		double result = 0;
		System.out
				.print("ASSUMING this is 0 because lambda usually 0 and r_o_sat is infinity: ");
		return Constants.s(result);
	}
