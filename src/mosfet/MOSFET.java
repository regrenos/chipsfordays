...
public class MOSFET {
	...
	public static double Vtn(double VBS, double Na, double Xox, double QfQit) {
		...
		double result = MOSCAP.Vfb(Na, Xox, QfQit)
				+ 2
				* phifb
				+ (1 / MOSCAP.Cox(Xox))
				* Math.sqrt(2 * Constants.q * Constants.esi * Na
						* ((2 * phifb) - VBS));
	...
	public static double gammaBn(double Na, double Xox) {
		double result = Math.sqrt(2 * Constants.q * Constants.esi * Na)
				/ MOSCAP.Cox(Xox);
		System.out
				.print("The bulk body effect coefficient for the MOSFET is: ");
		return Constants.s(result);
	}

	/** Vdsat. Drain to Source Saturation Voltage */
	public static double VDsat(double VGS, double VBS, double Na, double Xox,
			double QfQit) {
		...
		if (VGS <= vtn) {
			...
			System.out
					.println("BTW, this NMOSFET is ON, If V_DS < VDsat, linear range, else saturation range.");
		}
	...
	public static double IDsat(double lambda, double Kn, double VDS,
